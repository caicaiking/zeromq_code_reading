...
zmq::object_t::object_t(ctx_t *ctx_, uint32_t tid_) : ctx(ctx_),
                                                      tid(tid_)
{
}

zmq::object_t::object_t(object_t *parent_) : ctx(parent_->ctx),
                                             tid(parent_->tid)
{
}
...
void zmq::object_t::process_command(command_t &cmd_)
{
    ...
    case command_t::attach:
        process_attach(cmd_.args.attach.engine,
                       cmd_.args.attach.peer_identity ? blob_t(cmd_.args.attach.peer_identity,
...
{
    return ctx->find_endpoin t(addr_)
}
